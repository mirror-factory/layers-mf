...
export const metadata: Metadata = {
  title: "User Guide",
  description: "Learn how to get the most out of Dewey.",
};
...
const GETTING_STARTED = [
  {
    ...
    description:
      "Head to the signup page and create your account with email or sign in with Google. Once logged in, you will be guided through onboarding where you name your organization and invite team members. Your organization is your shared workspace \u2014 everything you add to Dewey is scoped to it.",
  },
  {
    step: 2,
    icon: Plug,
  ...
  },
  {
    ...
    description:
      "Open Chat and type a question about your content \u2014 for example, \"What were the key decisions from last week's meeting?\" Dewey searches across all your connected sources, finds the most relevant information, and gives you an answer with clickable citations so you can verify the source.",
  },
];
...
You can export any item as Markdown or JSON, view its version history, and see a content health score that tells you how fresh and complete the item is. Items from integrations are automatically re-synced when the source changes, so your library stays up to date without manual effort.`,
  },
  {
    ...
    icon: MessageSquare,
    title: "Chat",
    content: `Chat lets you ask questions about your content using natural language. Type a question and Dewey will search across all your sources using hybrid search (combining keyword matching and AI-powered semantic search) to find the most relevant information. Every answer includes clickable citations so you can jump to the original source.
...
Use the feedback buttons (thumbs up/down) on any response to help Dewey learn what is useful to you. This feedback feeds into your Ditto profile and improves future results. You can also view the tool calls the AI made behind the scenes to understand how it found its answer.`,
  },
  {
    id: "sessions",
    icon: FolderKanban,
    title: "Sessions",
    content: `Sessions are focused workspaces where you group related content together. Create a session for a project, a meeting series, or any topic you are actively working on. Link context items to a session manually, or let Dewey auto-link new items when it detects they are related.
...
Sessions help you stay organized without creating rigid folder structures. You can have as many sessions as you need, and the same content item can appear in multiple sessions. Think of them as smart lenses over your knowledge base.`,
  },
  {
    ...
    icon: Inbox,
    title: "Inbox",
    content: `The Inbox is where Dewey surfaces items that need your attention. When new content is processed, Dewey uses AI to determine if it contains action items, important decisions, or information you should review. Each inbox item is assigned a priority level (high, medium, low) based on its content and relevance.
...
You can also configure notification preferences to get a daily digest email summarizing your inbox at a time that works for you. The digest includes the most important items from the past 24 hours.`,
  },
  {
    ...
    icon: Plug,
    title: "Integrations",
    content: `Dewey connects to the tools your team already uses. Currently supported integrations include Google Drive (documents, spreadsheets, PDFs), Linear (issues and projects), Slack (channel messages), GitHub (issues and discussions), Discord (server messages), and Granola (meeting notes).
...
Each integration shows its sync progress, last sync time, and health status. Dewey uses webhooks to receive real-time updates from your tools, so new content appears within minutes of being created. The integration catalog page documents the content types, capabilities, and limits for each provider.`,
  },
  {
    id: "search",
    icon: Search,
    title: "Search",
    content: `Dewey uses hybrid search to find content across all your sources. This combines traditional keyword matching with AI-powered semantic search, so you can find things even when you do not remember the exact words. Results are ranked using a combination of relevance, freshness, and source trust weighting.
...
Search results include a snippet showing where your query matched, the source integration, and a relevance score. From the results, you can open the full item, link it to a session, or start a chat conversation about it.`,
  },
  {
    ...
    icon: Bot,
    title: "Ditto",
    content: `Ditto is your personal AI profile within Dewey. It learns your preferences over time by observing what you search for, click on, and find useful. This profile helps Dewey personalize your experience \u2014 from inbox ranking to search results to chat style.

Visit the Ditto page to see what Dewey has learned about your interests, working patterns, and communication preferences. You can manually edit any of these preferences if something does not look right. Ditto also powers the "For You" suggestions on your dashboard, proactively surfacing content you might find relevant.
...
Your Ditto profile is private to you \u2014 other team members cannot see your preferences. The profile is regenerated weekly to stay current with your evolving interests.`,
  },
  {
    ...
    icon: Settings,
    title: "Settings",
    content: `Settings let you customize how Dewey works for you and your team. Source trust weighting lets you adjust how much weight different sources get in search results \u2014 slide a source up to prioritize it or down to de-emphasize it. This affects both search ranking and chat answers.
    ...
    <div className="space-y-8 max-w-3xl">
      ...
      <div>
        ...
        <p className="text-muted-foreground text-sm">
          Everything you need to know to get the most out of Dewey.
        </p>
      </div>
      ...
      <div className="rounded-lg border bg-muted/50 p-5 text-center">
        <p className="text-sm text-muted-foreground">
          Need more help? Use the Chat feature to ask Dewey about its own
