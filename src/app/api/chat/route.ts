...
function getVisualInstructions(level: string): string {
  ...
  return [
    ...
    "",
    "TIPS: Wrap library code in <script> tags. Give containers unique IDs. Check library exists: if(typeof gsap!=='undefined').",
    "For Chart.js: create a <canvas id='myChart'> then new Chart(document.getElementById('myChart'), config) in a <script>.",
    "",
    "QUALITY RULES:",
    ...
    "",
    "STYLE GUIDE:",
    "- Dark background (transparent body, parent is dark)",
