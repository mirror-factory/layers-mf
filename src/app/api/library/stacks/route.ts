...
import { createStack, listStacks } from "@/lib/library/domain";

export async function GET() {
  const auth = await requireUserAndOrg();
  if (isAuthFailure(auth)) return auth.response;

  const result = await listStacks(auth.supabase, auth.orgId);
  if ("error" in result) {
...
export async function POST(request: NextRequest) {
  ...
  const result = await createStack(auth.supabase, {
    orgId: auth.orgId,
    userId: auth.user.id,
    name: body.name.trim(),
