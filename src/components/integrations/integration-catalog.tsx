...
const PROVIDERS: ProviderInfo[] = [
  ...
  },
  {
    ...
    contentTypes: [
      { format: "Calendar events", description: "Event title, description, attendees, start/end time, location, and Google Meet links" },
    ],
    capabilities: [
      { icon: RefreshCw, label: "Sync events from 30 days ago to 30 days ahead" },
      { icon: Clock, label: "Event timing and attendee metadata preserved" },
      { icon: Search, label: "Full hybrid search across all synced events" },
      { icon: Brain, label: "AI extracts action items and key topics from event descriptions" },
      { icon: Zap, label: "Auto-links events to meeting transcripts and related context" },
    ],
    syncMethod: "OAuth via Nango → Google Calendar API v3 → AI pipeline",
    limits: "Up to 100 events per sync (30-day window), 12,000 chars per event",
    webhook: false,
  },
  {
    id: "notion",
    ...
    howItWorks: [
      ...
      "Incremental sync via last_edited_time change detection",
    ],
    contentTypes: [
      { format: "Pages", description: "Page content converted from Notion blocks to plain text — headings, paragraphs, lists, toggles, callouts, code blocks, quotes, and to-dos" },
      { format: "Database entries", description: "Database row properties extracted with page content combined into a single document" },
    ],
    capabilities: [
      { icon: RefreshCw, label: "Incremental sync via last_edited_time change detection" },
      { icon: Search, label: "Full-text + semantic search across all Notion content" },
      { icon: Brain, label: "AI extraction: entities, summaries, action items from pages" },
      { icon: Zap, label: "Auto-links new content to relevant sessions" },
    ],
    syncMethod: "OAuth via Nango → Notion API → block-by-block fetch → AI pipeline",
    limits: "Up to 50 pages per sync, nested blocks to depth 2, 12,000 chars per page, pages under 30 chars skipped",
    webhook: false,
  },
  {
    id: "gmail",
    ...
    howItWorks: [
      ...
      "Each email runs through the AI pipeline (extract → embed → search)",
    ],
    contentTypes: [
      { format: "Email messages", description: "Subject, sender (From), recipients (To), date, and plain text body. Multipart emails use the text/plain part." },
      { format: "Email threads", description: "Thread ID is preserved in metadata for grouping related messages" },
    ],
    capabilities: [
      { icon: RefreshCw, label: "Fetch up to 50 messages, 30 processed per sync" },
      { icon: Search, label: "Full hybrid search across all synced emails" },
      { icon: Brain, label: "AI extracts action items, decisions, and key information" },
      { icon: Zap, label: "Auto-links email content to relevant sessions" },
      { icon: Clock, label: "7-day rolling window keeps context fresh" },
    ],
    syncMethod: "OAuth via Nango → Gmail API v1 → message fetch → AI pipeline",
    limits: "Up to 50 messages listed, 30 processed per sync, 12,000 chars per email, emails under 30 chars skipped",
    webhook: false,
  },
];
