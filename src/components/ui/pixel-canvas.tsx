...
const PixelCanvas = React.forwardRef<HTMLDivElement, PixelCanvasProps>(
  ({ gap, speed, colors, variant, noFocus, style, ...props }, ref) => {
    ...
    React.useEffect(() => {
      ...
      // Create and mount custom element imperatively
      const el = document.createElement("pixel-canvas") as HTMLElement
      if (gap != null) el.dataset.gap = String(gap)
      if (speed != null) el.dataset.speed = String(speed)
      if (colors) el.dataset.colors = colors.join(",")
      if (variant) el.dataset.variant = variant
      if (noFocus) el.dataset.noFocus = ""
