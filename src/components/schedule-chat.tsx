...
function ScheduleToolCard({ part }: { part: ToolPart }) {
  ...
  if (part.type === "tool-schedule_action") {
    ...
    const result = output as { status?: string; name?: string; schedule?: string } | undefined;
    if (result?.status === "created" || result?.name) {
      return (
        <div className="flex items-center gap-2 rounded-lg border border-emerald-500/30 bg-emerald-500/5 px-3 py-2 text-xs my-1">
          ...
          <span>Schedule created{result.name ? `: ${result.name}` : ""}</span>
        </div>
      );
    }
    return null;
  ...
  // list_schedules: loading indicator
  if (part.type === "tool-list_schedules") {
    if (!isDone) {
      return (
        <div className="flex items-center gap-2 text-xs text-muted-foreground py-1.5">
          <List className="h-3.5 w-3.5 animate-pulse" />
  ...
  // edit_schedule: editing a schedule
  if (part.type === "tool-edit_schedule") {
    if (!isDone) {
      return (
        <div className="flex items-center gap-2 text-xs text-muted-foreground py-1.5">
    ...
    if (result?.status === "updated" || result?.name) {
      return (
        <div className="flex items-center gap-2 rounded-lg border border-blue-500/30 bg-blue-500/5 px-3 py-2 text-xs my-1">
          <Pencil className="h-3.5 w-3.5 text-blue-500 shrink-0" />
          <span>Schedule updated{result.name ? `: ${result.name}` : ""}</span>
        </div>
      );
    }
    return null;
  ...
  if (part.type === "tool-delete_schedule") {
    if (!isDone) {
      return (
        <div className="flex items-center gap-2 text-xs text-muted-foreground py-1.5">
          <Loader2 className="h-3.5 w-3.5 animate-spin" />
    ...
    if (result?.status === "deleted") {
      return (
        <div className="flex items-center gap-2 rounded-lg border border-amber-500/30 bg-amber-500/5 px-3 py-2 text-xs my-1">
          <Trash2 className="h-3.5 w-3.5 text-amber-500 shrink-0" />
          <span>Schedule deleted{result.name ? `: ${result.name}` : ""}</span>
        </div>
      );
    }
    return null;
  ...
  return (
    <div className="rounded-lg border bg-card overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-4 py-3 border-b bg-muted/30">
        ...
        <span className="text-sm font-medium">Schedule Assistant</span>
        <span className="text-[10px] text-muted-foreground">Create and manage scheduled tasks</span>
      </div>
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-xs text-muted-foreground">
              Ask me to create or manage scheduled tasks.
            </p>
            <p className="text-[11px] text-muted-foreground/60 mt-1">
              Try &quot;Create a daily digest&quot; or &quot;Show my schedules&quot;
            </p>
          </div>
        )}
              ...
              <div className="space-y-1">
                {message.parts?.map((part, i) => {
                  ...
                  if (part.type.startsWith("tool-")) {
                    return <ScheduleToolCard key={i} part={part as ToolPart} />;
                  }
                  return null;
                })}
              </div>
      ...
      <div className="border-t px-3 py-2 flex items-end gap-2">
        <textarea
          ...
          placeholder="Create a schedule, check existing ones..."
          rows={1}
