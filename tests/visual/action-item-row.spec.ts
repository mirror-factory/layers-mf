/**
 * Auto-scaffolded by sync-registries.ts for action-item-row.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/action-item-row.spec.ts
 *
...
test.describe("visual: action-item-row", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering action-item-row.
    await page.goto("/");
    await expect(page).toHaveScreenshot(
      `action-item-row-${testInfo.project.name}.png`,
