/**
 * Auto-scaffolded by sync-registries.ts for label.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/label.spec.ts
 *
...
test.describe("visual: label", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering label.
    await page.goto("/");
    await expect(page).toHaveScreenshot(`label-${testInfo.project.name}.png`, {
      animations: "disabled",
