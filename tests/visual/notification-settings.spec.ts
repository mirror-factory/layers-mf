/**
 * Auto-scaffolded by sync-registries.ts for notification-settings.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/notification-settings.spec.ts
 *
...
test.describe("visual: notification-settings", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering notification-settings.
    await page.goto("/");
    await expect(page).toHaveScreenshot(
      `notification-settings-${testInfo.project.name}.png`,
