/**
 * Auto-scaffolded by sync-registries.ts for switch.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/switch.spec.ts
 *
...
test.describe("visual: switch", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering switch.
    await page.goto("/");
    await expect(page).toHaveScreenshot(`switch-${testInfo.project.name}.png`, {
      animations: "disabled",
