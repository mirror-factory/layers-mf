/**
 * Auto-scaffolded by sync-registries.ts for sphere.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/sphere.spec.ts
 *
...
test.describe("visual: sphere", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering sphere.
    await page.goto("/");
    await expect(page).toHaveScreenshot(`sphere-${testInfo.project.name}.png`, {
      animations: "disabled",
