/**
 * Auto-scaffolded by sync-registries.ts for create-session-dialog.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/create-session-dialog.spec.ts
 *
...
test.describe("visual: create-session-dialog", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering create-session-dialog.
    await page.goto("/");
    await expect(page).toHaveScreenshot(
      `create-session-dialog-${testInfo.project.name}.png`,
