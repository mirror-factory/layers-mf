/**
 * Auto-scaffolded by sync-registries.ts for dialog.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/dialog.spec.ts
 *
...
test.describe("visual: dialog", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering dialog.
    await page.goto("/");
    await expect(page).toHaveScreenshot(`dialog-${testInfo.project.name}.png`, {
      animations: "disabled",
