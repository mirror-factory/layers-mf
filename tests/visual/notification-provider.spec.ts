/**
 * Auto-scaffolded by sync-registries.ts for notification-provider.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/notification-provider.spec.ts
 *
...
test.describe("visual: notification-provider", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering notification-provider.
    await page.goto("/");
    await expect(page).toHaveScreenshot(
      `notification-provider-${testInfo.project.name}.png`,
