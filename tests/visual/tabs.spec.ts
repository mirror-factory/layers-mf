/**
 * Auto-scaffolded by sync-registries.ts for tabs.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/tabs.spec.ts
 *
...
test.describe("visual: tabs", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering tabs.
    await page.goto("/");
    await expect(page).toHaveScreenshot(`tabs-${testInfo.project.name}.png`, {
      animations: "disabled",
