/**
 * Auto-scaffolded by sync-registries.ts for context-window-bar.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/context-window-bar.spec.ts
 *
...
test.describe("visual: context-window-bar", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering context-window-bar.
    await page.goto("/");
    await expect(page).toHaveScreenshot(
      `context-window-bar-${testInfo.project.name}.png`,
