/**
 * Auto-scaffolded by sync-registries.ts for chat-participants-modal.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/chat-participants-modal.spec.ts
 *
...
test.describe("visual: chat-participants-modal", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering chat-participants-modal.
    await page.goto("/");
    await expect(page).toHaveScreenshot(
      `chat-participants-modal-${testInfo.project.name}.png`,
