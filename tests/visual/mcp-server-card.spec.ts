/**
 * Auto-scaffolded by sync-registries.ts for mcp-server-card.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/mcp-server-card.spec.ts
 *
...
test.describe("visual: mcp-server-card", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering mcp-server-card.
    await page.goto("/");
    await expect(page).toHaveScreenshot(
      `mcp-server-card-${testInfo.project.name}.png`,
