/**
 * Auto-scaffolded by sync-registries.ts for skill-reference-files.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/skill-reference-files.spec.ts
 *
...
test.describe("visual: skill-reference-files", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering skill-reference-files.
    await page.goto("/");
    await expect(page).toHaveScreenshot(
      `skill-reference-files-${testInfo.project.name}.png`,
