/**
 * Auto-scaffolded by sync-registries.ts for source-trust-settings.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/source-trust-settings.spec.ts
 *
...
test.describe("visual: source-trust-settings", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering source-trust-settings.
    await page.goto("/");
    await expect(page).toHaveScreenshot(
      `source-trust-settings-${testInfo.project.name}.png`,
