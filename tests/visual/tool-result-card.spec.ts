/**
 * Auto-scaffolded by sync-registries.ts for tool-result-card.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/tool-result-card.spec.ts
 *
...
test.describe("visual: tool-result-card", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering tool-result-card.
    await page.goto("/");
    await expect(page).toHaveScreenshot(
      `tool-result-card-${testInfo.project.name}.png`,
