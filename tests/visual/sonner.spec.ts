/**
 * Auto-scaffolded by sync-registries.ts for sonner.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/sonner.spec.ts
 *
...
test.describe("visual: sonner", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering sonner.
    await page.goto("/");
    await expect(page).toHaveScreenshot(`sonner-${testInfo.project.name}.png`, {
      animations: "disabled",
