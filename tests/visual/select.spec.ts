/**
 * Auto-scaffolded by sync-registries.ts for select.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/select.spec.ts
 *
...
test.describe("visual: select", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering select.
    await page.goto("/");
    await expect(page).toHaveScreenshot(`select-${testInfo.project.name}.png`, {
      animations: "disabled",
