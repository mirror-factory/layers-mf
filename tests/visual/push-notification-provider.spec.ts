/**
 * Auto-scaffolded by sync-registries.ts for push-notification-provider.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/push-notification-provider.spec.ts
 *
...
test.describe("visual: push-notification-provider", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering push-notification-provider.
    await page.goto("/");
    await expect(page).toHaveScreenshot(
      `push-notification-provider-${testInfo.project.name}.png`,
