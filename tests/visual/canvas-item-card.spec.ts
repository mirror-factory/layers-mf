/**
 * Auto-scaffolded by sync-registries.ts for canvas-item-card.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/canvas-item-card.spec.ts
 *
...
test.describe("visual: canvas-item-card", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering canvas-item-card.
    await page.goto("/");
    await expect(page).toHaveScreenshot(
      `canvas-item-card-${testInfo.project.name}.png`,
