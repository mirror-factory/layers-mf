/**
 * Auto-scaffolded by sync-registries.ts for slider.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/slider.spec.ts
 *
...
test.describe("visual: slider", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering slider.
    await page.goto("/");
    await expect(page).toHaveScreenshot(`slider-${testInfo.project.name}.png`, {
      animations: "disabled",
