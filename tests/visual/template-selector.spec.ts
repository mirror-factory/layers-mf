/**
 * Auto-scaffolded by sync-registries.ts for template-selector.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/template-selector.spec.ts
 *
...
test.describe("visual: template-selector", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering template-selector.
    await page.goto("/");
    await expect(page).toHaveScreenshot(
      `template-selector-${testInfo.project.name}.png`,
