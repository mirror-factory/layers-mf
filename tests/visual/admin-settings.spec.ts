/**
 * Auto-scaffolded by sync-registries.ts for admin-settings.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/admin-settings.spec.ts
 *
...
test.describe("visual: admin-settings", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering admin-settings.
    await page.goto("/");
    await expect(page).toHaveScreenshot(
      `admin-settings-${testInfo.project.name}.png`,
