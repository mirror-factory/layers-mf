/**
 * Auto-scaffolded by sync-registries.ts for card.
 *
 ...
 *
 *   VISUAL_UPDATE=1 pnpm exec playwright test tests/visual/card.spec.ts
 *
...
test.describe("visual: card", () => {
  test("matches baseline", async ({ page }, testInfo) => {
    // TODO: point at a Storybook URL / dedicated test page rendering card.
    await page.goto("/");
    await expect(page).toHaveScreenshot(`card-${testInfo.project.name}.png`, {
      animations: "disabled",
