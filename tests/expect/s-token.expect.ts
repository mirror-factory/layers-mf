/**
 * Auto-scaffolded by scaffold-expect-tests.ts for /s/[token].
 *
 ...
 * when ready.
 *
 * Run: pnpm exec expect tests/expect/s-token.expect.ts
 */
import { test, expect } from '@anthropic-ai/expect';

test.describe.skip('expect: /s/[token]', () => {
  test('user can load the page', async ({ page }) => {
    await page.goto('/s/sample');
