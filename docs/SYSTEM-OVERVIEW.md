...
|-------|----------|---------|
| Voice listening | Medium | Simplified state flow pushed but untested end-to-end |
| Voice TTS response | Blocked | Cartesia wired but can't verify until STT works |
| Highlight after doc switch | Medium | Race condition --- works if PDF already loaded, fails on fresh switch |
...
| Flash Lite tool looping | Architectural | Gemini 3.1 Flash Lite struggles with 20+ tools, requires intent detection narrowing |
...
1. **Verify voice mode end-to-end** --- STT -> chat -> TTS response
2. **Fix highlight-after-doc-switch race condition** --- wait for PDF load before applying highlights
3. **Upgrade portal model** from Gemini 3.1 Flash Lite to Gemini 3 Flash for production (reduces need for intent detection)
